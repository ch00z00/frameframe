...
- An Astro project, most likely at the repo root alongside the existing `docs/` and `.claude/`
  directories (not yet decided whether the dev-log tooling needs to move if this creates
  ambiguity with Astro's own build output — revisit at spec-design time if it does).
- Article content as MDX files under an Astro **content collection**, one file per article,
  with a frontmatter schema covering at least: `title`, `tags`, `is_premium` (see
  `product.md`), and any director/actor/location references used as in-article supporting
  data (per the decision to not give those their own pages).
- A small, growing **custom component library** (comparison tables, pull-quotes, image
  essays, timelines, etc.) importable from within MDX article bodies, per the decision to
  keep per-article visual presentation distinctive rather than templated.
- Styling as CSS custom properties (design tokens), not a utility framework — see `tech.md`.
